...
public:
	...
	void checkCity(string& city) {
		if (!isCityExist(city)) 
			throw exception("“акого города не существует!");
		if (!isCItyNotUsed(city)) 
			throw exception("“акой город уже называли!");
		if (!isLetterFits(city))
			throw exception("√ород, начинающийс€ с такой буквы, не подходит!.");
	...
	char getLastLetter() const {
		...
		int i = 1;
		while (string("ыьъЄй").find(letter) != string::npos) {
	...
	const string& wordToLowerCase(string& str) {
		string upperCase = "…÷” ≈Ќ√Ўў«’Џ‘џ¬јѕ–ќЋƒ∆Ёя„—ћ»“№Ѕё®";
		string lowerCase = "йцукенгшщзхъфывапролджэ€чсмитьбюЄ";
